        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="body">
                    ...
                    <form action="{{ route('admin.reports.transections') }}" method="get">
                    @else
                    <form action="{{ route('author.reports.transections') }}" method="get">
                    @endif
                        <div class="col-md-3">
                            <div class="form-group form-float">
                            
                                <select name="employee_id"  class="form-control show-tick" data-live-search="true" required>
                                    <option value="">Select Employee</option>
                                    @foreach( $employees as $data)
                                    <option value="{{ $data->id }}" >{{ $data->name }} - {{ $data->emply_id }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td> {{ $data->stock->product->type == 'laptop' ? "RSC-".$data->stock->serial_no : "" }}</td>
                                    ...
                                    <td>
                                        @if(Auth::user()->role_id == 1)
                                        <a href="{{ route('admin.reports.show', $data->employee) }}" class="btn btn-info waves-effect " >
                                            <i class="material-icons">visibility</i>
                                        </a>
                                        @else
                                        <a href="{{ route('author.reports.show', $data->employee) }}" class="btn btn-info waves-effect " >
                                            <i class="material-icons">visibility</i>
                                        </a>
                                        @endif
